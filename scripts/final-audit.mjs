...
function walk(root) {
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) walk(path); else if (/\.(tsx|ts|css)$/.test(entry.name)) files.push(path);
  }
}
roots.forEach(walk);
...
const technicalLeak = traveler.filter(({ text }) => /BrainState|BrainController|agentResults|providerRegistry|orchestrator/i.test(text));
...
if (directStatusColors.length > 12) failures.push(`too many direct status-color files: ${directStatusColors.length}`);
if (technicalLeak.length > 1) failures.push(`technical vocabulary detected in traveler surfaces: ${technicalLeak.map((item) => item.file).join(", ")}`);
