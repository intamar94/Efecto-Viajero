...
async function weatherFor(destination: ResolvedDestination, start?: string, end?: string): Promise<WeatherSnapshot> {
  ...
  const response = await fetch(url, { next: { revalidate: 1800 } }); if (!response.ok) throw new Error(`No se pudo obtener el tiempo de ${destination.name}`);
...
export interface AgentProviderInput { requirementId?: string; dataType?: string; question?: string; dependencySignals?: ProviderSignal[]; }
...
export async function executeTask(task: ResearchTask, context: CanonicalTripContext, locations: ResolvedDestination[], input: AgentProviderInput = {}): Promise<ResearchResult[]> {
  ...
  const baseInput = {
    start: context.dates.start, end: context.dates.end, currency: context.budget.moneda,
    budgetAmount: context.budget.importe, budgetType: context.budget.tipo,
    travelerCounts: { adults: context.travelers.adultos ?? 0, children: context.travelers.ninos ?? 0, babies: context.travelers.bebes ?? 0, seniors: context.travelers.personasMayores ?? 0, pets: context.travelers.mascotas ?? 0 },
    ...input,
  };
