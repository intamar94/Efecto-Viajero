...
export interface DepartmentExecution { results: ResearchResult[]; reports: DepartmentReport[]; availableDomains: string[]; unavailableDomains: string[]; neuralCycles: NeuralCycle[]; }
...
/** Executes the reverse-engineered requirement graph and lets incomplete neurons trigger focused follow-up work. */
export async function runDepartments(plan: ResearchPlan, context: CanonicalTripContext, locations: ResolvedDestination[], reversePlan?: ReverseEngineeringPlan): Promise<DepartmentExecution> {
  if (!reversePlan) return { results: [], reports: [], availableDomains: [], unavailableDomains: plan.selectedDomains, neuralCycles: [] };

  let requirements = [...reversePlan.requirements];
  let agents = [...reversePlan.agents];
  const allResults = new Map<string, AgentResult>();
  const neuralCycles: NeuralCycle[] = [];

  for (let cycle = 1; cycle <= 3; cycle++) {
    const pending = requirements.filter(r => !allResults.has(r.id));
    if (!pending.length) break;
    const resultWave = await executeAgents(pending, agents.filter(a => pending.some(r => r.agentId === a.id)), context, locations, []);
    resultWave.forEach(r => allResults.set(r.requirementId, r));
    const neural = buildNeuralCycle(pending, resultWave, cycle);
    neuralCycles.push(neural);
    const next = materializeFollowUpRequirements(neural.followUps, requirements, agents);
    if (next.requirements.length === requirements.length) break;
    requirements = next.requirements;
    agents = next.agents;
  }

  const agentResults = [...allResults.values()];
  ...
  return { results, reports, availableDomains: [...available], unavailableDomains: [...unavailable], neuralCycles };
}
