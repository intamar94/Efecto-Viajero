...
async function buildBrainState(context: CanonicalTripContext, analysis: Awaited<ReturnType<typeof analyzeTrip>>): Promise<BrainState> {
  const requirements = analysis.reverseEngineering.requirements; const agents = analysis.reverseEngineering.agents; let results = analysis.departmentReports.flatMap((report) => report.agentResults);
  ...
  let conflictResolutions = resolveWorkingMemoryConflicts(analysis.workingMemory.conflicts, claims); let blockers = buildBlockers(analysis); let decision = decideNextAction(context, actionState.pending, results, conflictResolutions);
  ...
  const executor = createBrainActionExecutor({ locations: analysis.locations }); const controlCycles = [...state.controlCycles];
  for (let cycle = 1; cycle <= MAX_CONTROL_CYCLES; cycle++) {
    decision = state.decision ?? decideNextAction(context, state.pendingActions, state.results, conflictResolutions); const action = decision.action;
    ...
    const execution = await executor.execute(state, action); const merged = new Map(state.results.map((result) => [result.requirementId, result])); execution.results.forEach((result) => merged.set(result.requirementId, result)); results = [...merged.values()];
    absorbAgentResults(analysis.workingMemory, execution.results); actionState = markValidatedActions(deriveBrainActions(requirements, results, analysis.workingMemory.conflicts), results); claims = new Map(results.map((result) => [result.requirementId, { requirementId: result.requirementId, value: result.data, evidence: result.evidence ?? [] }])); conflictResolutions = resolveWorkingMemoryConflicts(analysis.workingMemory.conflicts, claims);
  ...
  const marketingDesign = buildMarketingDesignBrief(finalState); const designSystem = buildDesignSystemBrief(finalState); const baseCreativeState = updateBrainState(finalState, { marketingDesign, designSystem });
  const sourceFiles = collectCreativeAuditSources(); const creativeAudit = auditCreativeAgents({ state: baseCreativeState, sourceFiles }); const creativeDepartment = attachCreativeAuditToPlan(buildCreativeDepartmentPlan(updateBrainState(baseCreativeState, { creativeAudit })), creativeAudit);
  const implementationTasks = buildImplementationQueue(creativeDepartment.instructions, creativeAudit.findings); const verificationResults = verifyCreativeReport(baseCreativeState, creativeAudit, sourceFiles);
  ...
  const memoryStore = createSupabaseBrainMemoryStore(); if (memoryStore) { try { await memoryStore.save(snapshotBrain(completeState)); } catch (error) { console.warn("Brain memory persistence unavailable", error); } }
...
export async function runBrain(input: BrainInput): Promise<BrainRun> {
  ...
  const deconstructed = deconstructTripText(text); const analysis = await analyzeTrip(text, context); const brain = await buildBrainState(context, analysis); return { context, deconstructed, analysis, brain };
}
