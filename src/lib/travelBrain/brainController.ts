...
function buildBlockers(analysis: Analysis): BrainBlocker[] { return [...analysis.unresolved.map(target => ({ id: `unresolved:${target}`, type: "missing-data" as const, target, reason: target, severity: "high" as const })), ...analysis.unavailableDomains.map(target => ({ id: `provider:${target}`, type: "provider" as const, target, reason: `La capacidad ${target} no está disponible en esta ejecución.`, severity: "high" as const })), ...analysis.workingMemory.conflicts.map(conflict => ({ id: `conflict:${conflict.key}`, type: "conflict" as const, target: conflict.key, reason: conflict.reason, severity: "high" as const }))]; }
function phaseFor(action: BrainAction | null | undefined, blockers: BrainBlocker[], complete: boolean): BrainPhase { if (complete) return "complete"; if (blockers.some(b => b.severity === "critical")) return "blocked"; if (!action) return blockers.length ? "blocked" : "complete"; if (action.type === "resolve_conflict") return "resolving"; if (["research", "verify", "cross_check", "request_missing_data"].includes(action.type)) return "researching"; if (action.type === "recalculate") return "applying"; return "deciding"; }
function markValidatedActions(actions: BrainAction[], results: AgentResult[]) { const validated = new Set(results.filter(result => result.status === "ready" && result.validation.valid).map(result => result.dataType)); const pending: BrainAction[] = []; const completed: BrainAction[] = []; for (const action of actions) (action.type === "research" && validated.has(action.target) ? completed : pending).push(action.type === "research" && validated.has(action.target) ? { ...action, status: "completed" } : action); return { pending, completed }; }
...
async function buildBrainState(context: CanonicalTripContext, analysis: Analysis): Promise<BrainState> {
  ...
  for (let cycle = 1; cycle <= MAX_CONTROL_CYCLES; cycle++) {
    ...
    controlCycles.push({ cycle, phase: phaseFor(action, blockers, false), decisionId: decision.id, selectedActionId: action.id, selectedActionType: action.type, selectedTarget: action.target, outcome: executed ? "completed" : execution.execution.status === "failed" ? "blocked" : "waiting", reason: execution.execution.reason, createdAt: new Date().toISOString() });
    ...
    if (!executed) { state = updateBrainState(state, { terminationReason: "max-cycles" }); break; }
  }
