...
function markValidatedActions(actions: BrainAction[], results: AgentResult[]): { pending: BrainAction[]; completed: BrainAction[] } {
...
function buildBrainState(context: CanonicalTripContext, analysis: Awaited<ReturnType<typeof analyzeTrip>>): BrainState {
  ...
  const brain = createBrainState({ runId: `brain:${Date.now()}`, context, requirements, agents, locations: analysis.locations });
  ...
  return updateBrainState(state, { changeSets: [...state.changeSets, changeSet] });
}
