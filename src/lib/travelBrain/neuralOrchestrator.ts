...
export function materializeFollowUpRequirements(followUps: NeuralFollowUp[], existing: DataRequirement[], agents: AgentSpec[]): { requirements: DataRequirement[]; agents: AgentSpec[] } {
  ...
  for (const f of followUps) {
    ...
    newRequirements.push(requirement); newAgents.push({ id: requirement.agentId, name: `${f.domain}.${f.dataType}`, domain: f.domain, input: ["TripContext", "validated_signals", "dependency_results", "previous_evidence"], output: [f.dataType, "evidence", "validation", "confidence", "freshness"], requirementIds: [f.id], mode: "research" }); existingIds.add(f.id);
  }
