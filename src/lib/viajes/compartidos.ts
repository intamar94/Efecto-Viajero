...
export async function invitar(viajeId:string,email:string){
  ...
  const{data,error:e}=await (supabase.from("viales_compartidos") as any).insert({viaje_id:viajeId,propietario_id:user.id,invitado_email:email.trim().toLowerCase()}).select().single();
