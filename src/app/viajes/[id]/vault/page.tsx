...
import { impactosDelDocumento } from "@/lib/documentIntelligence";
            ...
            {grupos.map(({ categoria, documentos }) => (
              <section key={categoria}>
                ...
                <ul className="space-y-2">
                  {documentos.map((d) => {
                    const impactos = impactosDelDocumento(d);
                    return (
                      <li key={d.id} className={`rounded-xl border bg-white px-4 py-3 text-sm ${d.id === ultimoGuardado ? "border-coral-300 ring-1 ring-coral-100" : "border-neutral-200"}`}>
                        <div className="flex items-start justify-between gap-3">
                          <span className="font-medium">{d.proveedor}</span>
                          <button onClick={() => eliminar(d.id)} className="shrink-0 text-xs text-neutral-400 hover:text-red-600">Eliminar</button>
                        </div>
                        {(d.referencia || d.fecha || d.hora) && <p className="mt-0.5 text-neutral-500">{[d.referencia, d.fecha, d.hora].filter(Boolean).join(" · ")}</p>}
                        {d.direccion && (
                          <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(d.direccion)}`} target="_blank" rel="noopener noreferrer" className="mt-0.5 inline-block text-marino-600 underline hover:text-marino-800">
                        ...
                        {d.autoClasificado && (
                          <label className="mt-2 flex flex-wrap items-center gap-2 border-t border-neutral-100 pt-2 text-xs text-neutral-400">
                            ...
                            <select value={d.tipo} onChange={(e) => cambiarCategoria(d.id, e.target.value as CategoriaDocumento)} className="rounded-lg border border-neutral-200 bg-white px-2 py-1 text-xs text-neutral-700">
                              {ORDEN_CATEGORIAS.map((c) => <option key={c} value={c}>{ETIQUETA_CATEGORIA[c].etiqueta}</option>)}
                            </select>
                          </label>
                        )}
                      </li>
