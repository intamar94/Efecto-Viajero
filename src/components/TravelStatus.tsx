...
export function TravelStatus({ status, className = "" }: { status: PresentationStatus; className?: string }) {
  ...
  return (
    <span className={`travel-status travel-status-${status} inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium ${className}`} role="status" aria-label={item.label}>
